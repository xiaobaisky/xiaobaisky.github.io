...
let sco = {
    ...
    initThemeColor: function() {
        window.addEventListener('scroll', utils.throttle(function (e) {   
            ...
            if (sco.is_Post()) {
                if (currentTop == 0) {
                    let e = getComputedStyle(document.documentElement).getPropertyValue("--efu-main");
                    sco.changeThemeColor(e)
                } else{
                    let e = getComputedStyle(document.documentElement).getPropertyValue("--efu-card-bg");
                    sco.changeThemeColor(e)
                }
            }else if (currentTop == 0) {
                let e = getComputedStyle(document.documentElement).getPropertyValue("--efu-background");
                sco.changeThemeColor(e)
            }else{
                let e = getComputedStyle(document.documentElement).getPropertyValue("--efu-card-bg");
                sco.changeThemeColor(e)
