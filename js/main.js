...
const sco = {
    ...
    initThemeColor: function () {
        if (sco.is_Post()) {
            const currentTop = window.scrollY || document.documentElement.scrollTop
            if (currentTop === 0) {
                let themeColor = getComputedStyle(document.documentElement).getPropertyValue('--efu-main');
                sco.changeThemeColor(themeColor);
            } else {
                let themeColor = getComputedStyle(document.documentElement).getPropertyValue('--efu-background');
                sco.changeThemeColor(themeColor);
            }
        } else {
            let themeColor = getComputedStyle(document.documentElement).getPropertyValue('--efu-background');
...
function navTitle() {
    ...
    var simptitle = titlevalue.replace(' | Hai sky', '')
    document.getElementById("page-name-text").innerHTML = simptitle;
}
function firstChild() {
    // 获取父元素

    var parentElement = document.getElementById('card-hotpost');
    
    // 获取父元素的第一个子元素
    var firstChilds = parentElement.firstElementChild || parentElement.firstChild;
    
    // 为第一个子元素添加样式
    firstChilds.firstChild.classList.add('first-child-style');
    //自定义结束
}
...
window.refreshFn = () => {
    ...
    if(document.getElementById('card-widget-calendar')) {
        cardTimes();
    }
    // nav中间的标题变化
    navTitle();
